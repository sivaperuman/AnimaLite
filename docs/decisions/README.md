...
| [DEC-0015](DEC-0015-classical-comparator.md) | The classical warp/flow comparator is block matching in NumPy, in-repo, not `minterpolate`; it is the credible non-learned reference, compared on the same inputs, and cannot itself qualify | implementation | Accepted | §6.0, MR-018, MR-016, AT-055/056 |
| [DEC-0016](DEC-0016-execution-admission.md) | Executing third-party model artifacts requires a recorded, artifact- and purpose-bound admission decision; missing/pending/rejected all block before any invocation, with no bypass | implementation (mechanism) / **owner + legal** (the decisions it records) | Accepted | CR-024, C-04, MR-012, MR-014, D-06 |
| [DEC-0017](DEC-0017-runtime-provisioning-and-thread-accounting.md) | `runtime status` inspects bytes instead of executing them; `runtime fetch --install` is bounded, verify-before-extract and atomic; the `-j` allocation is derived from the declared budget and labelled as declared, not measured | implementation | Accepted | handoff rules 7 & 8, §12.0 envelope, NFR-028 |
